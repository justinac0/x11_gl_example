...
INTERNAL void gl_debug_callback_(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length,
...
GLOBAL GLctx gl_ctx_create(NativeWindow* window) {
    ...
    {  // Surface
        ...
        for (EGLint i = 0; i < config_count; i++) {
            ...
            ctx.surface = eglCreatePlatformWindowSurface(ctx.display, configs[i], &window->handle, attr);
